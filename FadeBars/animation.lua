---------------------------------------------------------------
-- Animation.lua: Taint-free animation replacement
---------------------------------------------------------------
-- Provides a replacement for managing alpha animations without
-- risk of spreading taint, which might happen when the "real"
-- functions are called from secure code.
-- Courtesy of MonkDev who got it from somewhere else and doesn't remember from where.
-- Modified to work with my addon in 8.0.1

local _, core = ...;
core.Anim = {};

local Anim = core.Anim;
---------------------------------------------------------------
...
function Anim.UIFrameFadeIn(test, frame, timeToFade, startAlpha, endAlpha)
	local framer = _G[frame]
	local fadeInfo = {}
    fadeInfo.mode = "IN"
    fadeInfo.timeToFade = timeToFade
    fadeInfo.startAlpha = startAlpha
    fadeInfo.endAlpha = endAlpha
    FadeFrame(framer, fadeInfo)
end
 
-- Convenience function for simple fade out
function Anim.UIFrameFadeOut(test, frame, timeToFade, startAlpha, endAlpha)
	local framer = _G[frame]
	local fadeInfo = {}
    fadeInfo.mode = "OUT"
    fadeInfo.timeToFade = timeToFade
...
Anim.UIFrameFlash =  function(frame, fadeInTime, fadeOutTime, flashDuration, showWhenDone, flashInHoldTime, flashOutHoldTime, syncId)
    if frame then
        local index = 1
